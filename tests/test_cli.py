...
class TestSearch:
    ...
    def test_search_lists_models_from_index(self, mock_index, runner):
        mock_index.return_value = [
            {"name": "whisper-stt", "tag": "base.en", "type": "stt", "description": "logical"},
            {"name": "kokoro-tts", "tag": "v1.0", "type": "tts", "description": "logical"},
        ]
        result = runner.invoke(cli, ["search"])
        assert result.exit_code == 0
        assert "whisper-stt:base.en" in result.output
        assert "kokoro-tts:v1.0" in result.output
