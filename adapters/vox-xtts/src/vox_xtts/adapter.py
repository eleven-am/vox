...
def _runtime_root() -> Path:
    return vox_runtime_root() / "xtts"


def _clear_modules(prefixes: tuple[str, ...]) -> None:
    purge_runtime_modules(prefixes)


def _ensure_runtime_path() -> str:
    runtime_root = _runtime_root()
    runtime_root.mkdir(parents=True, exist_ok=True)
    write_app_fallback_path(runtime_root)
    return activate_runtime_path(runtime_root, root=runtime_root.parent)
...
def _install_xtts_runtime() -> None:
    ...
    if install_target_runtime_requirements(
        runtime_path,
        ("coqui-tts>=0.27.5", "accelerate>=1.10.0,<2.0.0"),
        upgrade=False,
        timeout=1800,
        install_runner=_run_install_command,
        context="XTTS runtime install",
    ):
        ...
        *,
        ...
        if reference_audio is not None:
            if np.asarray(reference_audio, dtype=np.float32).size == 0:
                raise InvalidConfigError("XTTS reference_audio is empty")
            return
