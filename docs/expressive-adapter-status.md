...
| `indextts-tts:2` | `vox-indextts==0.1.14` | Covered by adapter tests; runtime under `$VOX_HOME/runtime/indextts`; purges sibling-runtime Transformers modules, stale Torch/CUDA runtime packages, and stale Matplotlib artifacts before import probes; selects constructor signatures without swallowing internal model-load failures; exposes IndexTTS2 emotion text/vector controls through Vox synthesis params | Linux x86_64 CUDA/Torch; CPU/ONNX and Spark/ARM NVIDIA not production-supported | Pending isolated GPU smoke |
